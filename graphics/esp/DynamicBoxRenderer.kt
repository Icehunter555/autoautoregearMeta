...
class DynamicBoxRenderer : AbstractIndexedRenderer() {
    ...
    inner class Builder {
        ...
        fun putBox(box: AxisAlignedBB, xOffset: Double, yOffset: Double, zOffset: Double, color: Int, sideMask: Int) {
            putBox(box, xOffset, yOffset, zOffset, color, sideMask, SideMask.toOutlineMask(sideMask))
        }

        fun putBox(box: AxisAlignedBB, xOffset: Double, yOffset: Double, zOffset: Double, color: Int, sideMask: Int, outlineMask: Int) {
            ...
            val minXD = box.minX - renderPosX
            val minYD = box.minY - renderPosY
            val minZD = box.minZ - renderPosZ
            val maxXD = box.maxX - renderPosX
            val maxYD = box.maxY - renderPosY
            val maxZD = box.maxZ - renderPosZ
            ...
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XN_YN_ZN)) putVertex(prevMinX, prevMinY, prevMinZ, minX, minY, minZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XN_YN_ZP)) putVertex(prevMinX, prevMinY, prevMaxZ, minX, minY, maxZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XN_YP_ZN)) putVertex(prevMinX, prevMaxY, prevMinZ, minX, maxY, minZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XN_YP_ZP)) putVertex(prevMinX, prevMaxY, prevMaxZ, minX, maxY, maxZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XP_YN_ZN)) putVertex(prevMaxX, prevMinY, prevMinZ, maxX, minY, minZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XP_YN_ZP)) putVertex(prevMaxX, prevMinY, prevMaxZ, maxX, minY, maxZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XP_YP_ZN)) putVertex(prevMaxX, prevMaxY, prevMinZ, maxX, maxY, minZ)
            if (BoxVertexMask.contains(vertexMask, BoxVertexMask.XP_YP_ZP)) putVertex(prevMaxX, prevMaxY, prevMaxZ, maxX, maxY, maxZ)
        ...
        fun putBox(box: AxisAlignedBB, xOffset: Double, yOffset: Double, zOffset: Double, color: Int) {
            val minXD = box.minX - renderPosX
            val minYD = box.minY - renderPosY
            val minZD = box.minZ - renderPosZ
            val maxXD = box.maxX - renderPosX
        ...
        private fun putVertex(prevX: Float, prevY: Float, prevZ: Float, x: Float, y: Float, z: Float) {
            posList.add(prevX); posList.add(prevY); posList.add(prevZ)
        ...
        private fun buildVboBuffer(): ByteBuffer {
            val buffer = GLAllocation.createDirectByteBuffer(vertexSizeInternal * 28)
            var vertexIndex = 0
    ...
    object Shader : DrawShader("/assets/meta/shaders/general/DynamicBoxRenderer.vsh", "/assets/meta/shaders/general/Renderer.fsh"), AlwaysListening {
